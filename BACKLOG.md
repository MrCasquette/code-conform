...
- **`/audit-site-vitrine`** v0.3 — refonte structurelle 2026-05-22 sur signal test 2 agay (trous de cartographie en audit en bloc : RGPD, getStaticPaths, doublons, aria manqués). Bascule vers **boucle par axe avec sub-agent ciblé** : le LLM principal n'inspecte plus le code en profondeur, il dérive sa propre checklist d'axes depuis la SSOT chargée puis délègue chaque axe à un sub-agent Explore avec briefing précis. Corrections par écart (un à la fois dans l'axe), validation utilisateur par diff. Audit honnête (anti-remplissage) ajouté `philosophy §8` + skill : si conforme dire conforme, ne pas forcer trouvaille. Test 1 v0.3 agay axe 1 réussi (citations verbatim, sub-agent focalisé, plan ciblé) — mais a **révélé une friction sur la SSOT elle-même** (cf. ligne suivante), pas sur le skill. Reste à mesurer : (a) test 1 complet sur agay une fois SSOT à jour, (b) déléguer cartographie surface au sub-agent vs LLM principal qui lit surface lui-même, (c) tenue de la posture sur axes pluri-écarts (10+ écarts dans un axe).
