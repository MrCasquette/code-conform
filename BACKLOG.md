...
- [ ] **`install.sh` : gérer les orphelins** — bug détecté 2026-05-20. Le script copie/lie les skills actuels du repo mais ne supprime pas les skills disparus du repo qui restent installés dans `~/.claude/skills/`. Conséquence : à chaque suppression de skill côté source, l'utilisateur doit nettoyer manuellement. Fix proposé : détection auto des dossiers portant la sentinelle `.installed-by-code-conform` mais absents du repo source → soit demande de confirmation interactive, soit flag `--prune` explicite. Vote : détection auto + confirmation, cohérent avec philosophy §8 (modification dangereuse → accord explicite).
