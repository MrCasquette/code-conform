...
- **`/audit-site-vitrine`** v0.3 — refonte structurelle 2026-05-22 sur signal test 2 agay (trous de cartographie en audit en bloc : RGPD, getStaticPaths, doublons, aria manqués). Bascule vers **boucle par axe avec sub-agent ciblé** : le LLM principal n'inspecte plus le code en profondeur, il dérive sa propre checklist d'axes depuis la SSOT chargée puis délègue chaque axe à un sub-agent Explore avec briefing précis. Corrections par écart (un à la fois dans l'axe), validation utilisateur par diff. Audit honnête (anti-remplissage) ajouté `philosophy §8` + skill : si conforme dire conforme, ne pas forcer trouvaille. Test 1 v0.3 agay axe 1 réussi (citations verbatim, sub-agent focalisé, plan ciblé) — mais a **révélé une friction sur la SSOT elle-même** (cf. bascule §10 ci-dessous), pas sur le skill. Test axe 2 agay 2026-05-22 a révélé une **2e friction symétrique** : le LLM principal a filtré silencieusement 2 findings borderline du sub-agent (point namespace + contradiction casts) dans son résumé initial — ils n'ont été remontés que sur question utilisateur. **À acter en v0.4** : posture *"reporting honnête du sub-agent"* en miroir de l'anti-remplissage — le LLM principal doit remonter intégralement les findings, y compris borderline/à arbitrer ; pas de filtrage silencieux. Reste à mesurer : (a) test complet axes 3 à 10 sur agay, (b) tenue posture spot-check (transparence + scope ≤4 fichiers ou redéléguer), (c) pluri-écarts dans un axe.
